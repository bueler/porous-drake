...
Suppose $\Omega$ is a 2D or 3D domain with well-behaved boundary.  We will use $x,y$ for the horizontal coordinates and $z$ for the vertical coordinate.  (In 2D we use only $x,z$.)  Note distances are measured in meters and $z$ is measured positive upward.

Within $\Omega$ we assume there is a matrix of porous material with spatially-variable porosity $\phi(x,y,z)$ and permeability $k(x,y,z)$, which are dimensionless and in (SI units of) $\text{m}^2$, respectively.  These fields are assumed independent of time, but they are not assumed to be smooth.  In fact we will be interested in cases where they have large discontinuities.

An ideal gas flows through the porous matrix, and we will take this gas to be ideal and isothermal.  The following are thus positive constants: $R$ is the gas constant, $T$ is the absolute temperature, $M$ is the molar mass, and $g$ is the acceleration of gravity.  The ideal gas law says the density $\rho$ and pressure $P$ are proportional via $c = M/(RT)$:
\begin{equation}
\rho = c P.  \label{eq:ideal}
\end{equation}

We assume that the gas flow satisfies Darcy's law \citep{Fowler2011}.  In such a model the volumetric flux $\bq$, also called the discharge, is driven by the gradient of the gas pressure.  However, in the presence of gravity the weight of the gas above contribute to the pressure even in the absence of flow.  Thus Darcy's law reads \citep{Collinson2012}:
\begin{equation}
\bq = - \frac{k}{\mu} \left(\grad P + \rho g \grad z\right) \label{eq:pmtime:darcy}
\end{equation}
Here $\mu$ is the dynamic viscosity of the gas, again taken to be constant.  Note that if the surface pressure is atmospheric ($\Patm$) then a no-flow condition corresponds to $P=\Patm + \rho g h$ where $h$ is the depth; this consequence of equation \eqref{eq:pmtime:darcy} is unrelated to the ideal gas equation.

Suppose gas is generated within the domain $\Omega$, for example by exsolution (degassing) of the porous matrix, at a rate of $Q_m(t,x,y,z)$ mass per volume per time.  Because mass is conserved, the following equation holds:
\begin{equation}
\phi \frac{\partial \rho}{\partial t} + \Div \left(\rho\, \bq\right) = Q_m. \label{eq:pmtime:masscont}
\end{equation}
Note that we may also write \eqref{eq:pmtime:masscont} using the (conserved) mass flux $\bsigma = \rho \bq$; in the time-independent and source-free case \eqref{eq:pmtime:masscont} reduces to $\Div \sigma = 0$.
...
Regarding units of the solution fields, $\rho(t,x,y,z)$ is in (SI units of) $\text{kg}\,\text{m}^{-3}$, $P(t,x,y,z)$ in $\text{Pa} = \text{N}\,\text{m}^{-2}$, $\bq(t,x,y,z)$ in $\text{m}^3\,\text{m}^{-2}\,\text{s}^{-1} = \text{m}\,\text{s}^{-1}$, and $\bsigma$ in $\text{kg}\,\text{m}^{-2}\,\text{s}^{-1}$.  The constant $c$ is in $\text{kg}\,\text{J}^{-1}$, and the dynamic viscosity $\mu$ is in $\text{Pa}\,\text{s}$.
